  ...
  capturePageSnapshot() {
    ...
    html2canvas(document.body, {
      ...
      onclone: function (clonedDoc) {
        const fallback = 'rgba(16,16,16,0.2)'
        ...
        clonedDoc.querySelectorAll('style').forEach(styleEl => {
          ...
          styleEl.textContent = styleEl.textContent
            ...
            .replace(/lab\([^;{}]*\)/gi, fallback)
        })

        const hardOverride = clonedDoc.createElement('style')
        hardOverride.textContent = `
          *, *::before, *::after {
            border-color: rgba(16,16,16,0.2) !important;
            outline-color: rgba(16,16,16,0.2) !important;
            text-decoration-color: currentColor !important;
          }
        `
        clonedDoc.head.appendChild(hardOverride)
      },
